...
export function useToEditOrAdd() {
  ...
  const toEdit = (row) => {
    router.push("/sys/user/edit/" + row.id);
  };
  const toAdd = () => {
    router.push("/sys/user/add");
  };
  const toTag = (row) => {
    router.push("/sys/tag/?user_id=" + row.id);
  };
  const toAddressBook = (row) => {
    router.push("/sys/addressBook/?user_id=" + row.id);
  };
  return {
    toAdd,
    toEdit,
...
export function useChangePwd() {
  const changePass = async (admin) => {
    ...
    const confirm = await ElMessageBox.confirm(
      T("Confirm?", { param: T("ResetPassword") }),
      {
        confirmButtonText: T("Confirm"),
        cancelButtonText: T("Cancel"),
      }
    ...
    const res = await changePwd({ id: admin.id, password: input.value }).catch(
      (_) => false
    );
    if (!res) {
      return;
